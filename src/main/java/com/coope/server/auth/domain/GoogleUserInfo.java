package com.coope.server.auth.domain;

import com.coope.server.user.domain.enums.Provider;

import java.util.Map;

public class GoogleUserInfo implements com.coope.server.auth.domain.OAuth2UserInfo {
    private final Map<String, Object> attributes;

    public GoogleUserInfo(Map<String, Object> attributes) {
        this.attributes = attributes;
    }

    @Override public com.coope.server.user.domain.enums.Provider getProvider() { return Provider.GOOGLE; } // Enum 활용!
    @Override public String getProviderId() { return (String) attributes.get("sub"); }
    @Override public String getEmail() { return (String) attributes.get("email"); }
    @Override public String getName() { return (String) attributes.get("name"); }
    @Override public String getPicture() { return (String) attributes.get("picture"); }
}
