package com.coope.server.document.application;
...
import com.coope.server.shared.error.exception.DocumentNotFoundException;
...
public class DocumentService {
    ...
    public List<DocumentResponse> getSidebarDocuments(String workspaceCode, Long parentId, User user) {
        ...
        return documentRepository.findAllByWorkspaceAndParentWithChildCheck(workspace.getId(), parentId)
                .stream()
    ...
    @Transactional
    public DocumentResponse createDocument(String workspaceCode, Long parentId,
                                           String title, String icon, String content, User user) {
        Workspace workspace = workspaceService.getByInviteCode(workspaceCode);
        workspace.validateEditor(user.getId());

        Document parentDocument = null;
        if (parentId != null) {
            parentDocument = documentRepository.findById(parentId)
                    .orElseThrow(() -> new DocumentNotFoundException("부모 문서를 찾을 수 없습니다. ID: " + parentId));
        ...
        Document document = Document.createDocument(title, icon, content, user, workspace, parentDocument);
        Document savedDocument = documentRepository.save(document);

        String redisKey = "document-snapshot:" + savedDocument.getId();
        redisTemplate.opsForValue().set(redisKey, "[]", 1, TimeUnit.HOURS);

        DocumentResponse response = DocumentResponse.of(savedDocument, false);
        broadcast(workspaceCode, "UPSERT", response);

        return response;
    }

    @Transactional
    public DocumentResponse update(Long documentId, String title, String icon, String coverImage, User user) {
        Document document = findDocumentById(documentId);
        document.getWorkspace().validateEditor(user.getId());

        if (title != null) document.updateTitle(title);
        if (icon != null) document.updateIcon(icon);
        ...
        if (coverImage != null) {
            ...
            document.updateCoverImage(coverImage);
            if (oldUrl != null && !oldUrl.equals(coverImage)) {
    ...
    private boolean isValidImageUrl(String url) {
        ...
        return url.contains(".s3.amazonaws.com") || url.contains("localhost:8080");
    }
}
