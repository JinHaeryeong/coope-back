...
public class S3FileService implements FileService {
    ...
    public String upload(MultipartFile file, ImageCategory category) {
        ...
        String s3Key = category.dir() + "/" + UUID.randomUUID() + extractExtension(file, category);
        try {
    ...
    public Resource loadAsResource(String fileUrl, ImageCategory category) {
        try {
            String path = new java.net.URI(
                    java.net.URLDecoder.decode(fileUrl, StandardCharsets.UTF_8)).getPath();
    ...
    public boolean deleteFile(String imageUrl, ImageCategory category) {
        ...
        try {
            String path = new java.net.URI(
                    java.net.URLDecoder.decode(imageUrl, StandardCharsets.UTF_8)).getPath();
